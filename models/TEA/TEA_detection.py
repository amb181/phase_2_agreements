...
# # # # # Folder path to test TEAs # # # # #

model_info_path = "/home/rasa-stakeholder-assistant/agreements/phase2/TEA_model_info/"
path_test = "/home/rasa-stakeholder-assistant/agreements/phase2/Teaming_Agreement_test/"
...
def train_nda_model():
    ...
    # Read csv file
    df = pd.read_csv('/home/rasa-stakeholder-assistant/agreements/phase2/TEA_training_clustered_V2.csv')
    df.head()
    ...
    pickle.dump(model, open(os.path.join(model_info_path+"TEA_model.sav"), 'wb'))
...
def predict_tea(agreement, classes, model, calibrated_svc,tfidf):
    # # # # # P R E D I C T I O N # # # # #
    send_to_att = False
    ...
    for line in lines:
        ...
        if line.strip():
            ...
            if not send_to_att:
                ...
                attachm_parag += paragraph
            paragraph = ''
    return sections, attachm_parag
...
def insert_into_DB(agreement, parties, scope, conf_info, employee, affiliates, duration, duty, exc_info, rights, compliance, notice, \
    ...
    try:
        cursor = db.cursor()
        sql = "INSERT INTO NDA (id, Agreement_name, Parties, Scope_of_Disclosure, Use_of_Confidential_Information, Employee_Access_and_Control_of_Information, \
        Affiliates, Duration_of_Protection, Duty_to_Protect, Excluded_Information, Rights_and_Obligations, \
        Compliance_with_Laws, Notice, Assignment, Amendments, Controlling_Law, Expiration_of_Agreement, \
        Injunctive_Relief, Attorneys_Fees, Counterparts_Facsimile_Delivery, Miscellaneous) \
        ...
        db.autocommit(True)
        db.close()
        print('\n-- NDA agreement successfully added to the DB. --\n')
...
if __name__ == "__main__":
    ...
    i = 0
    if train_process:
        classes, model, calibrated_svc,tfidf = train_nda_model()   # Train model
    else:
        # load the model from disk
        classes = pickle.load(open(os.path.join(model_info_path+"classes.sav"), 'rb'))
        model = pickle.load(open(model_info_path+"TEA_model.sav", 'rb'))
    ...
    for file in folder:
        if numbr in range (20,40):
            ...
            if process is "Classify":
                sections, attachm_parag = predict_tea(file, classes, model, calibrated_svc,tfidf) # Get sections detected
                ...
                print(pdf_name)
                try:
                    InsertIntoDB(pdf_name,classes,sections)
                except Exception as e:
                    _, _, exc_tb = sys.exc_info()
                    print("Exeception occured  line {} : {}".format(exc_tb.tb_lineno,e))
